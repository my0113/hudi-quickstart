...
public class AppClient {
    ...
    public void run() throws IOException, YarnException {
        ...
        GetNewApplicationResponse newApplicationResponse = newApp.getNewApplicationResponse();
        // 设置application对象运行上下文
        appContext.setApplicationId(newApplicationResponse.getApplicationId());
        // 设置作业名称
        ...
        // 设置本作业的AM所需的资源信息
        appContext.setResource(new ResourcePBImpl() {{setMemory(1);setVirtualCores(1);}});
        // 设置运行完成后清除token
        appContext.setCancelTokensWhenComplete(true);
        // 设置作业最大失败重试的次数
        appContext.setMaxAppAttempts(4);
        // 设置作业在重试期间是否还能访问此Container
        appContext.setKeepContainersAcrossApplicationAttempts(true);
        // 设置作业资源请求信息
        appContext.setAMContainerResourceRequest(null);
        // 设置作业的标签
        appContext.setApplicationTags(new HashSet<String>() {{add("");}});
        //appSubmissionCtx
        // TODO 添加本地资源
        ...
        int memory = 1024;
        int vCores = 2;
        appContext.setResource(Resource.newInstance(memory, vCores));
        // 设置任务类型
        appContext.setApplicationType("my-yarn-application");
        // 默认是false
